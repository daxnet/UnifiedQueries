...
namespace UnifiedQueries.Validators
{
    internal sealed class ExpressionNameTypeValidator : QuerySpecificationValidator
    {
        ...
        protected override void VisitExpression(Expression expression)
        {
            if (this.nameTypeDictionary.ContainsKey(expression.Name))
            {
                ...
                {
                    this.AddError(string.Format("The data type defined on the expression '{0}' ('{1}') is not consistent with the previous definition ('{2}').",
                        expression.Name, expression.Type, type));
                }
            }
            else
